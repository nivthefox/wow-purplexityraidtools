...
export function parseSpecializationData(specializationDataText, specListText) {
    ...
    while ((match = enumPattern.exec(specializationDataText)) !== null) {
        const [, name, id] = match;
        // Skip non-spec entries
        if (name === 'SPEC_NONE' || name === 'SPEC_PET') continue;
    ...
    // -----------------------------------------------------------------------
    // Extract all enum names in order from the array initializer.
    const specListEntries = [];
    const entryPattern = /\b([A-Z][A-Z_]+[A-Z])\b/g;
    // Skip the first line (array declaration) and parse enum references
    const bodyStart = specListText.indexOf('{');
    const body = specListText.slice(bodyStart);

    // Split by inner braces — each { ... } block is one class_id
    const classBlocks = body.match(/\{[^{}]*\}/g);
    if (!classBlocks) {
        throw new Error('Failed to parse sc_spec_list.inc class blocks');
    }

    // classBlocks[0] = class_id 0 (pets), classBlocks[1] = class_id 1 (warrior), etc.
    ...
    for (const [classId, enumNames] of classIdToEnums) {
        // Skip pet class (classId 0)
        if (classId === 0) continue;

        const classPrefix = findCommonPrefix(enumNames);
        // Class token: strip trailing underscore, remove internal underscores
        ...
        for (const enumName of enumNames) {
            ...
            // Spec name: everything after the class prefix, title-cased
...
function findCommonPrefix(names) {
    ...
    // Split each name into underscore-delimited parts
